...
public class GrillePlacement extends Grille implements IGrilleP {
	...
	public void deleteAllRandomNavires() {
		deleteCalledNavire((INavire)new PorteAvion());
		deleteCalledNavire((INavire)new Croiseur());
		deleteCalledNavire((INavire)new ContreTorpilleur());
		deleteCalledNavire((INavire)new SousMarin());
		deleteCalledNavire((INavire)new Torpilleur());
	}
	
	private void deleteCalledNavire(INavire nav) {
		for(INavire realNavire: mapnavire.get(nav.getEnav()) ) {
			int posX = realNavire.getLcaseNav().get(0).getPosX();
			int posY = realNavire.getLcaseNav().get(0).getPosY();
			for(int i=0; i<realNavire.getLongueur(); i++) {				
				CaseMer caseMer = new CaseMer(posX+(nav.isOri()?0:i),
						posY+(nav.isOri()?i:0));
				realNavire.getLcaseNav().remove(new CaseNavire(posX+(nav.isOri()?0:i),
						posY+(nav.isOri()?i:0),
						nav.getNvieCase()));
				this.setCaze(caseMer.getPosX(), caseMer.getPosY(), caseMer);
			}
			if(nav.getEnav().equals(ENavire.PorteAvion)) {
				for( int i=0; i<3; i++) {
					CaseMer caseMer = new CaseMer(posX+(nav.isOri()?0:i)+(nav.isOri()?1:0), 
							posY+(nav.isOri()?i:0)+(nav.isOri()?0:1));
					realNavire.getLcaseNav().remove(new CaseNavire(posX+(nav.isOri()?0:i)+(nav.isOri()?1:0), 
							posY+(nav.isOri()?i:0)+(nav.isOri()?0:1), 
							nav.getNvieCase()));
					this.setCaze(caseMer.getPosX(), caseMer.getPosY(), caseMer);
				}				
